...
var testTableForMarshal = make(map[float64]string)
...
func init() {
	...
	// fill testTable For Marshal/Unmarshal tests
	for f, s := range testTable {
...
func TestJSON(t *testing.T) {
	for _, s := range testTableForMarshal {
		var doc struct {
...
func TestDecimal_Gt(t *testing.T) {
	// decimal pair -> greater ?
	testCases := map[inp3]bool{
	...
	for key, val := range testCases {
		if key.a.Gt(key.b) != val {
			t.Errorf("Error: (%v, %v), received %v", key.a, key.b, val)
		}
	}
}

func TestDecimal_Ge(t *testing.T) {
	// decimal pair -> greater or equal ?
	testCases := map[inp3]bool{
...
func TestDecimal_Lt(t *testing.T) {
	...
	for key, val := range testCases {
		if key.a.Lt(key.b) != val {
			t.Errorf("Error: (%v, %v), received %v", key.a, key.b, val)
		}
	}
}

func TestDecimal_Le(t *testing.T) {
	// decimal pair -> greater or equal ?
	testCases := map[inp3]bool{
		inp3{NewFromFloat(2.000), NewFromFloat(2.000)}:   true,
		inp3{NewFromFloat(2.010), NewFromFloat(2.000)}:   false,
